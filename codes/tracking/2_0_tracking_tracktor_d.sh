#!/bin/bash
#sh 2_0_tracking_tracktor_d.sh {YYYYMMDDhhmmss} $SGE_TASK_ID $CPU
...
python ${wd}/codes/2_1_run_tracktor_d.py ${wd}/videos/${date}/${file}.mp4 ${wd}/video_images/${date}/${file}_image.jpg ${wd}/rawdata/${date}/ $3
echo 'Finished tracking by tracktor.'
ffmpeg -y -i ${wd}/rawdata/${date}/${file}_tracked.avi ${wd}/rawdata/${date}/${file}_tracked.mp4
python ${wd}/codes/2_2_fix_tracks_d.py ${wd}/rawdata/${date}/${file}_tracked.csv $3
python ${wd}/codes/2_3_integrate_info_d.py ${wd}/rawdata/${date}/${file}_tracked_fixed.tsv ${wd}/videos/${date}/${prefix}.tsv ${wd}/finaldata/${date}/${file}_tracked_fixed_final.tsv
